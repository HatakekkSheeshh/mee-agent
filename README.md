...
| **Ghi nhớ hội thoại (remember/forget)** | Agent tự lưu sự thật lâu dài user nêu trong chat ("gọi tôi là Ronaldo", "deadline 30/06") vào AgentBase — scope `user` (`user_prefs/<ms_oid>`) hoặc `project` (`project_facts/<meeting_id>`). Chạy ngầm, recall vào prompt các lượt sau. `forget_fact` để TẮT mà không xóa, `remember_fact` để bật lại |
...
Agent biến kiến thức nêu trong chat thành **bộ nhớ lâu dài, recall được** — vượt qua
"Xóa hội thoại" (chỉ xóa `chat_messages`). Lưu vào **AgentBase memory-records**, KHÔNG
phải `memory_events`.

**Phân vùng theo mục đích (đọc == ghi cùng actor):**

| scope | namespace | dùng cho |
|---|---|---|
| `user` | `user_prefs/<ms_oid>` | sự thật riêng user (danh xưng, sở thích) — vd "gọi tôi là Ronaldo" |
| `project` | `project_facts/<meeting_id>` | sự thật chia sẻ trong dự án (chia theo `meeting_id`, KHÔNG theo `ms_oid` để tránh silo) |

`project_facts/<meeting_id>` tách biệt với bản chắt lọc `project_facts/mee-user` (sync
projection), nên fact ghi nhớ không bao giờ đè lên `project_memory`.

**Cơ chế:**
- 2 tool tự chạy (KHÔNG cần duyệt), ghi **chạy ngầm** (fire-and-forget) nên không chặn
  lượt chat. Best-effort: thiếu `MEMORY_ID` → no-op, không bao giờ làm hỏng lượt chat.
- AgentBase **insert-only** (DELETE bị 403) → mỗi record có marker
  `[mee-fact scope=… key=<hash text> active=<1|0> author=<ms_oid> session=<sid>]`.
  `remember_fact` ghi `active=1`; `forget_fact` ghi record mới `active=0` cùng `key`.
- Recall (`list_fact_records`): newest-wins theo `key`, ẩn fact nếu record mới nhất là
  tombstone. Ghi nhớ lại = ghi `active=1` mới hơn → hiện lại.
- `load_context` nạp user-facts (kể cả khi chưa chọn cuộc họp) + project-facts của cuộc
  họp hiện tại vào khối **"Ghi nhớ"** trong prompt (giới hạn 20 mục mới nhất).

**Vòng đời ví dụ:** "gọi tôi là Ronaldo" → lưu `user_prefs/<oid>` → lượt sau agent gọi
Ronaldo → "đừng gọi Ronaldo nữa" → ẩn (không xóa) → "gọi Ronaldo lại" → hiện lại.
