...
def gen_mom_task(self, recording_id: str, ui_lang: str = "vi") -> dict:
    ...
    async def _run() -> dict:
        ...
        async with AsyncSessionLocal() as session:
            ...
            # save_results only FLUSHES recording.mom_json — the session owns
            # the transaction boundary, so we must commit here or the freshly
            # generated MoM is rolled back on session close (the reload then
