...
def _agent_system_prompt(state: ChatState) -> str:
    ...
    return (
        ...
        "- 'Meeting N' / 'Phiên N' / 'phiên thứ N' (vd 'Meeting 1', 'phiên 2') là một PHIÊN "
        "trong dự án HIỆN TẠI, KHÔNG phải dự án khác → lấy recording_id qua `list_recordings` "
        "rồi đọc bằng `recording_mom`. TUYỆT ĐỐI KHÔNG gọi `switch_meeting` cho 'Meeting N' "
        "(chỉ dùng `switch_meeting` khi user nêu TÊN RIÊNG của một dự án khác).\n"
