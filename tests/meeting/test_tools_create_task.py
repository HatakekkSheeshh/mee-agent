...
def test_build_task_items_stamps_description_on_every_item():
    items = tools.build_task_items(
        [
            {"item": "Deploy v1", "pic": "hieunq3", "deadline": "20/06/2026"},
            {"item": "Write docs", "pic": "hieunq3", "deadline": ""},
        ],
        description="Description generated by AI",
    )
    assert len(items) == 2
    assert all(it["description"] == "Description generated by AI" for it in items)


def test_build_task_items_description_defaults_empty():
    items = tools.build_task_items([{"item": "X", "pic": "a", "deadline": ""}])
    assert items[0]["description"] == ""


def test_build_agenda_task_items_explicit_description_overrides_detail():
    items = tools.build_agenda_task_items(
        [{"agenda": "Topic A", "description": "agenda detail"}],
        description="Description generated by AI",
    )
    assert items[0]["description"] == "Description generated by AI"
